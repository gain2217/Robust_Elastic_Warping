...
% [ H, ok, score ] = HM_ransac(X1, X2, 500, 0.04);
...
% n = size(X1_ok,2);
% H = HM_weighted(X1_ok, X2_ok, ones(n,1));
...
file_h = fopen([exp_path, '/', 'statistics.txt'], 'w');
fprintf(file_h, '%d %d %d %f', imsize1(2), imsize1(1), size(matches, 2), elapsedTime);
fclose(file_h);
